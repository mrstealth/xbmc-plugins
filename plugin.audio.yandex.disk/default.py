...
AUDIO_FORMATS = ['aif', 'iff', 'm3u', 'm4a', 'mid', 'mp3', 'mpa', 'ra', 'wav', 'flac']


class YandexDiskClient(object):
    def __init__(self):
        self.id = 'plugin.audio.yandex.disk'
...
class YandexDisk():
    ...
    def menu(self):
        if self.client.username and self.client.password:
            # FIXME: List only Video/Musik/Pictures directory ???
        ...
        for name, path in self.client.ll(path).iteritems():
            if self.client.isFile(path):
                ...
                if format in AUDIO_FORMATS:
                    ...
                    item = xbmcgui.ListItem(name, thumbnailImage=self.yandex_music, iconImage=self.yandex_music)
                    item.setInfo('music', { 'title': name, 'artist': 'unknown', 'album': 'unknown', 'genre': 'unknown', 'playcount': 0 })
